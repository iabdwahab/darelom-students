...
const grades: { id: string; label: string; students: HonorStudent[] }[] = [
  {
    ...
    students: [
      ...
      {
        name: "سماء محى دسوقى إبراهيم مصطفى",
        seatnumber: 10048,
        rank: 5,
        percentage: "92.94%",
      },
...
function RankIndicator({ rank }: { rank: number }) {
  if (rank === 1) return <Trophy className="size-4 text-yellow-500 fill-yellow-500" />;
  if (rank === 2) return <Medal className="size-4 text-gray-400 fill-gray-400" />;
  if (rank === 3) return <Medal className="size-4 text-amber-600 fill-amber-600" />;
  return <span className="text-xs text-muted-foreground font-reem-kufi w-4 text-center">{rank}</span>;
}
    ...
    <section className="container mx-auto px-4 py-16">
      ...
      <div className="text-center mb-10">
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold font-reem-kufi">لوحة الشرف</h2>
          <Award className="size-8 text-primary" />
        </div>
        <p className="text-muted-foreground font-tajawal max-w-xl mx-auto">أوائل الطلاب في كل فرقة دراسية — العام الدراسي ٢٠٢٦</p>
      </div>

      {/* 2×2 Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
        {grades.map((grade) => (
          <div key={grade.id} className="rounded-2xl border bg-card overflow-hidden">
            {/* Card Header */}
            <div className="bg-muted/50 px-5 py-4 border-b flex items-center justify-between">
              <h3 className="font-bold font-reem-kufi text-lg">{grade.label}</h3>
            ...
            <ul className="divide-y">
              {grade.students.map((student) => (
                ...
                >
                  ...
                  {/* Name */}
                  <span className={`flex-1 text-sm ${student.rank <= 3 ? "font-bold" : ""}`}>{student.name}</span>

                  {/* Percentage */}
                  <span className="font-reem-kufi text-sm text-muted-foreground shrink-0">{student.percentage}</span>
                </li>
              ))}
            </ul>
          </div>
