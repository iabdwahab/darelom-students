...
const LecturesRecordingsSelector = () => {
  ...
  useEffect(() => {
    ...
    getDoc(doc(firebaseDB, `${grade}/lectures_recordings`))
      .then((snapshot) => {
        ...
        if (data && Object.keys(data).length) {
          ...
          if (DataKeys.includes('mojalad_tasgilat')) {
            const orderedData = ['mojalad_tasgilat', ...DataKeys.filter((item) => item !== 'mojalad_tasgilat')];
  ...
  return (
    <>
      ...
      {isLoading ? <LoadingSpinner /> :
        <>
          {Object.keys(recordings).length ? <div className='row g-2 my-2'>
            {sortedRecordingsKeys.map((record, index) => {
              return (
                <div className='col-sm-6' key={index}>
                  <Button text={translate(record)} onClick={() => window.location.href = recordings[record]} />
                </div>
              )
            })}
          </div> : <p className='text-center fw-bold mt-4 fs-4'>عذرًا، لا تتوفر تسجيلات.</p>}
